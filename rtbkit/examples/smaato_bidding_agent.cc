 ...
 */
struct SmaatoTestBiddingAgent :
        public BiddingAgent
{
    SmaatoTestBiddingAgent(
    ...
    void init(const std::shared_ptr<ApplicationLayer> appLayer)
    {
        ...
        onBidRequest = bind(
                &SmaatoTestBiddingAgent::bid, this, _1, _2, _3, _4, _5, _6);
        ...
        addPeriodic("SmaatoTestBiddingAgent::pace", 10.0,
                [&] (uint64_t) { this->pace(); });
    ...
    void setConfig()
    {
        ...
        config.account = {"smaato_test", "fixprice"};
		...
		{	Creative creative(320, 50, "Mobile50x320Image", 50320);
			config.creatives.push_back(creative);
		}
		{	Creative creative(300, 50, "Mobile50x300Image", 50300);
			config.creatives.push_back(creative);
		} 
		{	Creative creative(216, 36, "Mobile36x216Image", 36216);
		config.creatives.push_back(creative);
		}
		{	Creative creative(168, 28, "Mobile28x168Image", 28168);
		config.creatives.push_back(creative);
		}

		//----Smaato config
		config.providerConfig["smaato"]["seat"] = "123";
		...
		for (auto & c : config.creatives) {
			...
			c.providerConfig["smaato"]["nurl"] = "http://52.90.48.233:8000/win?bidrequest_id=%{bidrequest.id}&imp_id=%{imp.id}&width=" + std::to_string(c.format.width) + "&height=" + std::to_string(c.format.height) + "&auction_price=${AUCTION_PRICE}&auction_id=${AUCTION_ID}&adspot_id=${AUCTION_IMP_ID}&AUCTION_BID_ID=${AUCTION_BID_ID}&AUCTION_SEATID=${AUCTION_SEAT_ID}&AUCTION_ADID=${AUCTION_AD_ID}";
    ...
    */
            ...
            const Json::Value & augmentations)
    {
        for (Bid& bid : bids) {
            ...
            bid.bid(availableCreative, MicroUSD(1000));
...
int main(int argc, char** argv)
{
    ...
    RTBKIT::SmaatoTestBiddingAgent agent(serviceProxies, "smaato-agent");
